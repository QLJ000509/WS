...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_epoches", default=2, type=int) #24
    parser.add_argument("--network", default="network.SCconv", type=str)
    parser.add_argument("--lr", default=0.01, type=float)
    parser.add_argument("--num_workers", default=12, type=int) #32
    ...
    parser.add_argument("--session_name", default="SCconv_", type=str)
    parser.add_argument("--crop_size", default=224, type=int)#448
    ...
    critersion = torch.nn.CrossEntropyLoss(weight=None, ignore_index=255, reduction='elementwise_mean').cuda()#elementwise_mean
    ...
    train_dataset = voc12.data.VOC12ClsDataset(args.train_list, voc12_root=args.voc12_root,
                        ...
                        imutils.RandomResizeLong(256, 512),#将图像H,W随机调整为256至512的大小，比例与原图一致
                        transforms.RandomHorizontalFlip(),#随机水平翻转
                        transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1),
                        np.asarray]),#亮度对比度饱和度色调
                        transform2=
                        imutils.Compose([imutils.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),#输入在（0，1）之间
                        imutils.RandomCrop(args.crop_size),#（448，448）
                        imutils.HWC_to_CHW]))

    train_data_loader = DataLoader(train_dataset, batch_size=args.batch_size,
                                   shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True)

    max_step = (len(train_dataset) // args.batch_size) * args.max_epoches  #31728
    ...
    if args.weights[-7:] == '.params':
        assert args.network == "network.SCconv"
        import network.resnet38d
        weights_dict = network.resnet38d_SCconv.convert_mxnet_to_torch(args.weights)
    else:
